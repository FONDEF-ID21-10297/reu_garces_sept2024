...
2.  Desarrollar un `modelo` para estimar el `potencial hídrico xilemático` ($\Psi_m$) en `cerezo` utilizando las variables derivadas de satélite (bandas, índices, polarizaciones).
...
4.  Desarrollar un `prototipo de plataforma web` en el cual se implemente el modelo de estimación de `potencial de turgor` y que entregue información de las `frecuencias`, `tiempos` y `cantidad de agua` a aplicar en el riego.
:::
...
* Indices de vegetación en la región VNIR/SWIR
...
![](img/importancia_metabolitos.png){width='50%', fig_align='center'}
...
* Enviar artículo científico sobre la predicción de $\Psi_p$ para publicación a journal "International Journal of Applied Earth Observation and Geoinformation"

* Postular a proyecto FONDEF Tecnológico con el objetivo de escalar
